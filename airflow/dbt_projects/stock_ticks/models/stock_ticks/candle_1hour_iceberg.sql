...
with source_data as (
    ...
    from {{ source('kafka', 'candle_1hour') }}

    {% if is_incremental() %}
        where window_start_ts >= date_add('hour', -1, COALESCE((select min(max_window_start_ts) from (select source, symbol, max(window_start_ts) as max_window_start_ts from {{ this }} group by  source, symbol)), from_iso8601_timestamp('1971-01-01T00:00:00.000')))
    {% endif %}
)
